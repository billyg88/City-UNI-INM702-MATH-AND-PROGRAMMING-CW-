...
class billy_nn:
    ...
    def infer_sample(self,sample):
        ...
        prediction = self.signal_propagation_test(sample) # no dropout or scaling is applied here
    ...
    def signal_propagation_test(self,sample):
          ...
          # no scaling happens during testing
...
### Modelling
 #Standard call gets you an MLP. So only 1 hidden layer.        
            
nn = billy_nn( architecture = [1024,200,100,10], bias = False, activation = 'RELU',  learning_rate = 0.0001, dropout = True,
               dropout_rate = 0.8)   # dropout rate is  the KEEP PROBABILITY quantity
