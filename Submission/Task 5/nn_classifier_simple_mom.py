...
A GREAT WORKING EXAMPLE OF AN NN 
...
This version of the NN uses momentum to speed up the Gradient Descent algorithm. 
No regularization takes place in this network.


This is used for task 5 which uses the emotion classification Dataset (Same as Task 6)
...
testing_labels = [int(label) for label in testing_labels]
...
batch_size = 1  #size of the mini-batch should be set to 1 for stochastic gradient descent
epochs = 50 
iteration_losses = [] 
epoch_accuracies = []
